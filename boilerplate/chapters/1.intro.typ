...
- 有限全序集常有最大最小元. 請擬以歸納證明之
  ...
  + 察 $abs(S) = N+1$, 令 $S' = S without {s}$.
    由前款知 $S'$ 有最大元 $M'$ 與最小元 $m'$, 則 $max S = max{M', s}$, $min S = min{m', s}$ 也. 即 $S$ 有最大最小元也.

界, 集不逾之境也. 凡集 $S subset.eq T$ 之元 $s$, 若有 $s <= M$ 者, 則稱 $M$ 爲 $S$ 一*上界*. 反之, 若 $M <= s$ 則喚作*下界*. 若上下界並存, 則謂之*有界*. 界不必屬於集也. 上界之最小者, 號曰*上確界*, 或曰最小上界, 記曰 $sup S$. 下界之最大者, 號曰*下確界*, 或曰最大下界, 記曰 $inf S$.
...
例以上界與上確界, 察其性質, 凡有二項, 一曰 $sup S$ 乃 $S$ 之上界也, 二曰凡其上界者必不小於 $sup S$ 也, 即最小之上界也.
請問偏序集恆有上界否?
...
== 關係
稱集 $R subset.eq A times B$ 爲集 $A$, $B$ 上之*二元關係*, 畧以關係. 若 $A = B$ 即 $R subset.eq A^2$ 則畧以 $A$ 上之關係. 若記 $a in A$ 與 $b in B$ 之滿足關係 $R$ 者以中綴表達式, 則曰 $a R b$:
$
...
$
  id_S := {(s, s) | s in S}
$
例如 $S = {suit.club, suit.diamond, suit.heart}$, $id_S = {(suit.club, suit.club), (suit.diamond, suit.diamond), (suit.heart, suit.heart)}$
...
則謂之*偏序關係*. 偏序關係之最小者, 唯有恆等關係也. 不難證明之.
+ $id$ 滿足自反性, 反對稱性, 傳遞性, 故爲偏序關係也.
