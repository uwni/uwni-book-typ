...
$
*約式*, 或曰*最簡分式,* 分式之子母互素者也.
例如 $1 slash 1$, $2\/3$, $5\/8$. 以其子母皆最小, 立爲 $QQ\/=$ 之代表元也.
稠性: $a $
...
$
$p$ 與 $q$ 皆偶數, 而 $p \/ q$ 非約式也. 故知 $l$ 非分數之屬也. 以Ἵππασος之初覺爲嚆失. 分數之不全始昭於天下也, 此所以分數不可以度量也.
...
$a_n$ 之值終將近於 $L$ 而或不之及. 不得知其及否也. 恣取正數 $epsilon$, 不論大小, 必存一處 $N$, 凡 $n$ 之後於 $N$ 者, $a_n$ 皆足近於 $L$. 其近幾何? 謂其差小於 $epsilon$ 耳. 所以謂之漸行漸近而不必駐於一處也.

=== 單調有界性之定理

數列之收斂者, 其極限必存焉. 以單調有界性之定理得知收斂而不得知其極限也. 欲察極限幾何, 則猶須探其值而後驗以定義也. 幸有各術如下以索極限也, 一曰夾逼定理, 二曰四則運算, 三曰 Stolz-Cesàro 定理也
...
$
爲*無窮級數*, 畧作*級數*. 其中凡 $a_n > 0$ 者, 謂之*正項級數*.
若 $s_n$ 收斂即曰級數收斂. $s_n$ 發散即謂之級數發散. 有諸據可以斷級數之斂散. 請道其詳.
